...
extension SingleSymbolViewController{
  func singleSymbol(){
    //        single 操作符将限制 Observable 只产生一个元素。如果 Observable 只有一个元素，它将镜像这个 Observable 。如果 Observable 没有元素或者元素数量大于一，它将产生一个 error 事件和一个Event事件。
    let subject = Observable.from([1,2,3,4])
    subject.single().subscribe { (number) in
      print(number)
    } onError: { (error) in
      print(error)
    } onCompleted: {
